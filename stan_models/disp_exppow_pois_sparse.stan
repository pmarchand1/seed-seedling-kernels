...
model {
    ...
    for (i in 1:nobs) {
        nseed[i] ~ poisson(mu[itrap[i], iyear[i]]);    
    }
}
...
generated quantities {
    ...
    
    {
        ...
        nnz = 0;
        for (i in 1:nobs) {
            log_lik[i] = poisson_lpmf(nseed[i] | mu[itrap[i], iyear[i]]);
            nseed_sim[i] = poisson_rng(mu[itrap[i], iyear[i]]);
            ll_sim[i] = poisson_lpmf(nseed_sim[i] | mu[itrap[i], iyear[i]]);
