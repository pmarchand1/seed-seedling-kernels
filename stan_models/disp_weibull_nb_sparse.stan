...
data {
    ...
    // Index of trap and year associated with each observation
    int<lower=1> itrap[nobs];
    int<lower=1> iyear[nobs];
...
model {
    ...
    for (i in 1:nobs) {
        nseed[i] ~ neg_binomial_2(mu[itrap[i], iyear[i]], theta);    
    }
}
...
generated quantities {
    ...
    
    {
        ...
        nnz = 0;
        for (i in 1:nobs) {
            log_lik[i] = neg_binomial_2_lpmf(nseed[i] | mu[itrap[i], iyear[i]], theta);
            nseed_sim[i] = neg_binomial_2_rng(mu[itrap[i], iyear[i]], theta);
            ll_sim[i] = neg_binomial_2_lpmf(nseed_sim[i] | mu[itrap[i], iyear[i]], theta);
